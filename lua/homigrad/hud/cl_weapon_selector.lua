...
-- так эта хуйня взята с эскейпа от руби
...
function WS.DrawGradientText(text, font, posX, posY, textAlign)
    local t = RealTime() * 4
    local chars = GetTextChars(text)
    
    surface.SetFont(font)
    local totalWidth = surface.GetTextSize(text)
    local startX = posX - totalWidth * 0.5
    local acc = 0
    ...
    for i, char in ipairs(chars) do
        ...
        draw.DrawText(char, font, startX + acc + 2, posY + 2, ColorAlpha(color_black, WS.Transparent*255), TEXT_ALIGN_LEFT)
        draw.DrawText(char, font, startX + acc, posY, ColorAlpha(col, WS.Transparent*255), TEXT_ALIGN_LEFT)
...
local AcsentColor = Color(128,128,128)
...
function WS.WeaponSelectorDraw( ply )
    ...
    for i = 0, #Weapons do
        ...
        WS.DrawText( i+1, "HomigradFontMedium", position + sizeX/2, scrH*0.02, ColorAlpha(Color(150,150,150),WS.Transparent*255) ,TEXT_ALIGN_CENTER )
        ...
        for Id = 0, #slotTbl do
            ...
            surface.SetDrawColor( 128, 128, 128, WS.Transparent*( SelectedWep == wep and 200 or 0 )  )
            ...
            sizeHi = sizeHi + 2.5
            if SelectedWep == wep then
                WS.DrawGradientText( WS.GetPrintName(wep), "HomigradFontSmall", position + sizeX/2, sizeHi, TEXT_ALIGN_CENTER )
            else
